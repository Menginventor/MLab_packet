...
MLab_MPU6886::MLab_MPU6886() {}

void MLab_MPU6886::I2C_Read_NBytes(uint8_t driver_Addr, uint8_t start_Addr,
...
void MLab_MPU6886 ::I2C_Write_NBytes(uint8_t driver_Addr, uint8_t start_Addr,
...
void MLab_MPU6886 ::enableWakeOnMotion(Ascale ascale, uint8_t thresh_num_lsb) {
  uint8_t regdata;
...
void MLab_MPU6886 ::readAll() {
  I2C_Read_NBytes(MPU6886_ADDRESS, MPU6886_ACCEL_XOUT_H, 14, sensorBuf);

}
bool MLab_MPU6886 ::available() {
  uint8_t buf[1];
  I2C_Read_NBytes(MPU6886_ADDRESS, MPU6886_INT_STATUS, 1, buf);
  
  return bitRead(buf[0], 0);
}

void MLab_MPU6886 ::getAccelAdc(int16_t* ax, int16_t* ay, int16_t* az) {
  uint8_t buf[6];
...
void MLab_MPU6886 ::getGyroAdc(int16_t* gx, int16_t* gy, int16_t* gz) {
  uint8_t buf[6];
...
void MLab_MPU6886 ::getTempAdc(int16_t* t) {
  uint8_t buf[2];
...
void MLab_MPU6886 ::SetGyroFsr(Gscale scale) {
...
void MLab_MPU6886 ::SetAccelFsr(Ascale scale) {
  unsigned char regdata;
  regdata = (scale << 3);
  I2C_Write_NBytes(MPU6886_ADDRESS, MPU6886_ACCEL_CONFIG, 1, &regdata);
  delay(10);
...
void MLab_MPU6886 ::getAccelData(float * ax, float * ay, float * az) {
  int16_t accX = 0;
  int16_t accY = 0;
  int16_t accZ = 0;
...
void MLab_MPU6886 ::getGyroData(float * gx, float * gy, float * gz) {
  int16_t gyroX = 0;
  int16_t gyroY = 0;
  int16_t gyroZ = 0;
...
void MLab_MPU6886 ::getTempData(float * t) {
  int16_t temp = 0;
  getTempAdc(&temp);
...
void MLab_MPU6886 ::SetINTPinActiveLogic(uint8_t level) {
  uint8_t tempdata;
...
void MLab_MPU6886 ::DisableAllIRQ() {
