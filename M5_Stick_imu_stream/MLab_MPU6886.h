...
*/
#ifndef _MLabMPU6886_H_
#define _MLabMPU6886_H_
...
class MLab_MPU6886
{
  ...
  public:
    ...
    void getAres();
    int16_t raw_accX() {
      uint16_t res = sensorBuf[0] << 8 | sensorBuf[1];
      return res;
    }
    int16_t raw_accY() {
      uint16_t res = sensorBuf[2] << 8 | sensorBuf[3];
      return res;
    }
    int16_t raw_accZ() {
      uint16_t res = sensorBuf[4] << 8 | sensorBuf[5];
      return res;
    }
    int16_t raw_temp() {
      uint16_t res = sensorBuf[6] << 8 | sensorBuf[7];
      return res;
    }
     int16_t raw_gyroX() {
      uint16_t res = sensorBuf[8] << 8 | sensorBuf[9];
      return res;
    }
    int16_t raw_gyroY() {
      uint16_t res = sensorBuf[10] << 8 | sensorBuf[11];
      return res;
    }
    int16_t raw_gyroZ() {
      uint16_t res = sensorBuf[12] << 8 | sensorBuf[13];
      return res;
    }

  private:
    uint8_t sensorBuf[14];
};
#endif
